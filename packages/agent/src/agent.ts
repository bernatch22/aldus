...
function systemPrompt(doc: DocGraph): string {
  ...
  return [
    ...
    'contenido completo del documento como un grafo: cada nodo de texto con su',
    '`id`, posición y tamaño; cada imagen y campo con su `id` y rect.',
    '',
    'Cómo trabajás:',
    '- PREGUNTAS sobre el contenido → respondé directo leyendo el grafo. NO hay',
    '  tool de lectura: ya tenés todo el documento acá.',
    '- CAMBIOS → usá las tools de edición (edit_text, move_text, set_text_color,',
    '  set_text_size, delete_text, move_image, delete_image) referenciando los',
    '  `id` EXACTOS del grafo. Podés encadenar varias. No inventes ids.',
    '',

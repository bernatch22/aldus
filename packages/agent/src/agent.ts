...
export function systemPrompt(doc: DocGraph, page?: number): string {
  ...
  return [
    ...
    '',
    'CONVERTIR "XXXX" (o un hueco marcado) EN ALGO COMPLETABLE — elegí UNA estrategia,',
    'NUNCA las dos a la vez (línea + input duplican el hueco y rompen el layout):',
    '· CON INPUT (add_form_field): NO escribas "____" en el texto. Los guiones bajos',
    '  suelen NO estar en el subset de la fuente embebida → el bake cae a una fuente',
    '  estándar y las métricas/negrita cambian (texto desalineado). Reemplazá el XXXX',
    '  por ESPACIOS (misma cantidad de caracteres — el espacio sí está en el subset,',
    '  la fuente y el ancho se conservan) y poné el widget CUBRIENDO el hueco:',
    '  x = donde empezaba el XXXX, width ≈ chars×fontSize×0.5 (el ancho del XXXX),',
    '  height ≈ fontSize+4, y = baseline−3. El borde del widget ya marca el campo.',
    '· SIN INPUT (solo línea visual): reemplazá por "______" con ancho acorde',
    '  (~ancho_en_pt/(fontSize*0.5) guiones) — pero SOLO si el segmento no es de una',
    '  fuente subset-bold (si el estilo dice bold y hay riesgo, preferí el input).',
    'MANTENÉ EL ESTILO: nunca agregues bold/italic que el texto original no tenía.',
    'SÉ CONSCIENTE DEL EMPUJE: si el reemplazo es MÁS ANCHO que el original y el',
    'texto que sigue en el MISMO renglón se solaparía, corré ese texto a la derecha',
    '(move_text con dx = el crecimiento); si no hay lugar, avisá en vez de pisar.',

...
function systemPrompt(doc: DocGraph, pages?: number[], siblingsPages?: number[]): string {
  ...
  return [
    ...
    '- EJECUTÁ, no propongas: listar los cambios en texto NO modifica el documento.',
    '  Podés llamar tools en esta misma respuesta — no hay ningún modo "solo texto".',
    '  Si el texto extraído trae artefactos de encoding (caracteres raros, acentos',
    '  partidos, palabras cortadas en tramos), es NORMAL en PDFs de Word/LibreOffice:',
    '  NO es una página corrupta y NO es motivo para no editar — los placeholders y',

/**
 * pdf/contentWalk.ts — máquina de estado de texto (ISO 32000 §9.4) sobre los
 * ops del content stream: para cada operador de show-text (Tj/TJ/'/") calcula
 * su posición ABSOLUTA (Tm × CTM), fuente, tamaño y estado (Tc/Tw/Tz), con el
 * rango de bytes exacto para extirparlo/re-emitirlo.
 *
 * Layer 1, protocolo TONTO: reporta hechos, no decide negocio. Trasplante de
 * v1 bake/textWalk.ts con DOS ajustes exactos del plan:
 *  (a) mul/invert/Matrix viven en common/matrix.ts (acá solo se consumen).
 *  (b) el predicado "¿este fill es contenido?" NO se importa de color: llega
 *      INYECTADO en `opts.isContentFill` (default: TODO es contenido). El
 *      brain del bake (F3) le pasa el predicado real — `raw => !isWhite(raw)`
 *      de common/rawFill — para que el backstop salte el "papel" blanco.
 *  `walkTextOps` (wrapper muerto de v1) no cruzó a v2 por decisión del plan.
 *
...
import { IDENTITY, mul, type Matrix } from '../common/matrix.js';
...
export interface FillRectOp {
  ...
  fillColorRaw: string;
  /** CTM vigente en el punto del path — para re-emitir coords locales al mover. */
  ctm: Matrix;
}
...
export interface ContentWalk {
   ...
   *  dibuja contenido real (fill que pasa `isContentFill`, Do, BT, sh) — es
   *  decir DESPUÉS del "papel" (los fills blancos full-page con que muchos
   *  generadores pintan la hoja, si el predicado inyectado los excluye).
   *  Insertar en el byte 0 dejaría el bloque bajo ese papel opaco.
...
export interface ContentWalkOptions {
  /** ¿El fill crudo (`raw` verbatim, '' = negro) pinta CONTENIDO real?
   *  Default: todo es contenido (protocolo tonto). El bake inyecta
   *  `raw => !isWhite(raw)` (common/rawFill) para que el papel blanco
   *  full-page no cuente como contenido al computar el backstop. */
  isContentFill?: (raw: string) => boolean;
}

export function walkContent(src: Uint8Array, opts?: ContentWalkOptions): ContentWalk {
  const isContentFill = opts?.isContentFill ?? (() => true);
  ...
  for (const rec of ops) {
    switch (rec.op) {
      ...
      case 'f': case 'F': case 'f*': case 'b': case 'b*': case 'B': case 'B*': {
        consumeClip();
        if (isContentFill(fillColorRaw)) markContent(pathStart ?? { offset: rec.start, ctm });
        ...
        if (simple?.valid) {
          ...
          if (r) fillRects.push({ start: simple.start, end: rec.end, ...r, fillColorRaw: simple.fill, ctm: simple.ctm });
        }
        pathStart = null;
        simple = null;
        break;

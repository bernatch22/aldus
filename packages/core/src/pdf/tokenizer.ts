/**
 * pdf/tokenizer.ts — tokenizador del content stream de PDF (ISO 32000 §7.2/7.8).
 * Layer 1, protocolo tonto: cero decisiones de negocio.
 *
 ...
 *
 * Trasplante VERBATIM de v1 bake/tokenizer.ts. Conserva las DOS defensas
 * anti-OOM pagadas con un tab muerto (PDF taggeado de LibreOffice):
 *  1. `parseDict` entiende hex strings dentro del dict (/ActualText<FEFF00FA>).
 *  2. `parseKeyword` garantiza PROGRESO sobre un delimitador huérfano.
 */
...
export function tokenizeContentStream(src: Uint8Array): OpRecord[] {
  ...
  const parseString = (): Token => {
    ...
    while (i < n && depth > 0) {
      ...
      if (c === 0x5c) { // backslash
        ...
        else if (e !== undefined && e >= 0x30 && e <= 0x37) { // octal (1-3 dígitos)
          let v = e - 0x30;
  ...
  const parseDict = (): Token => {
    ...
    while (i < n && depth > 0) {
      ...
      // Hex string DENTRO del dict (p.ej. /Span<</ActualText<FEFF00FA>>>BDC de
      // un PDF taggeado): sin esto, el `>` que cierra el hex se apareaba con un
      // `>` del dict → el dict "cerraba" un byte antes y dejaba un `>` huérfano
      // a nivel top (que además colgaba parseKeyword — ver abajo).
      if (src[i] === 0x3c) { i++; while (i < n && src[i] !== 0x3e) i++; i++; continue; }
  ...
  const parseKeyword = (): Token => {
    const start = i;
    while (i < n && !isWs(src[i]!) && !isDelim(src[i]!)) i++;
    // PROGRESO GARANTIZADO: un delimitador huérfano (`>` `)` `]` `}`) caía acá
    // y el while no daba ni una vuelta → keyword vacío con end===start → el
    // loop externo repetía el mismo byte PARA SIEMPRE (OOM, tab muerto). Se
    // consume ese byte como keyword basura: el walker ignora ops que no conoce.
    if (i === start) i++;

/**
 * canvas/imagePixels.ts — píxeles REALES de cada imagen del PDF (con su
 * transparencia), sacados de `page.objs` de pdf.js (v1:
 * `apps/editor/src/editor/imagePixels.ts`). El ghost de arrastre los usa en
 * vez de recortar el snapshot de la página: un crop del snapshot arrastra el
 * FONDO que la imagen tenía detrás (otra imagen), y en las zonas transparentes
 * de un PNG ese fondo se ve como "un pedazo de imagen pegado". Con los
 * píxeles propios el ghost es exacto y sin halo.
 *
 * AJUSTE v2: cache de módulo global → instancia `ImagePixelCache`
 * (IDisposable) inyectada por la composition root, una por documento —
 * `dispose()` reemplaza el `clearImagePixelCache()` manual de v1.
 */
import type { IDisposable, ImageNode } from '@aldus/core';
...
export interface PixelSourcePage {
  objs: PdfObjs;
}

export class ImagePixelCache implements IDisposable {
  /** imageId → dataURL PNG (preserva alpha). null = no se pudo (máscara,
   *  inline, aún no resuelta) → el caller cae al crop del snapshot. */
  ...
  extract(page: PixelSourcePage, images: ImageNode[]): Map<string, string> {
    const out = new Map<string, string>();
    for (const img of images) {
      if (this.cache.has(img.id)) {
        const v = this.cache.get(img.id);

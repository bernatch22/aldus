/**
 * core/helpers.ts — helpers compartidos por el overlay (v1:
 * `apps/editor/src/editor/overlay/helpers.ts`): el logger gateado, el volcado
 * de debug de estilos, el estilo del contenedor editable y el clamp de
 * arrastre. Sin React (audit §2 COPY): `containerStyle` devuelve un record
 * plano en vez de `React.CSSProperties` — estructuralmente compatible, así
 * los boxes de editor-react lo asignan directo a `style={...}`.
 */
import { createLogger, type SegmentEdit, type SegmentNode } from '@aldus/core';
import { bucketFallback, dominantRun, family } from './text/styledDom.js';
import { stableFontFamily } from './text/fontRegistry.js';
...
/** Estilo inline del CONTENEDOR editable — record plano (asignable a
 *  `React.CSSProperties` sin importar `react` acá). */
...
export function containerStyle(seg: SegmentNode, edit: SegmentEdit | null, scale: number): ContainerStyle {

/**
 * imageKind — la IMAGEN del grafo como `INodeKind`. `ImageBox` VERBATIM de v1
 * (`overlay/ImageBox.tsx`): seleccionar, arrastrar (mover), grip (escalar),
...
/** v1 `effectiveImageRect` = el genérico `effectiveRect` de core v2. */
const effectiveImageRect = (img: ImageNode, edit: ImageEdit | null) => effectiveRect(img, edit);
...
function ImageKindBox({ ctx, node: img }: { ctx: OverlayCtx; node: ImageNode }) {
  return (
    <ImageBox
      ...
      scale={ctx.scale}
      selected={ctx.selectedId === img.id || ctx.multiSel.has(img.id)}
      edit={ctx.imageEdits.get(img.id) ?? null}
      isLocked={ctx.locked.has(img.id)}
      snapshot={ctx.snapshot}
      cleanPixels={ctx.imagePixels.get(img.id) ?? null}
      onSelect={() => ctx.selectNode(img.id)}
      onPatch={patch => ctx.ledger.patchRect(img, patch)}
      onDragging={ctx.onDragging}
    />
  );
}

export const imageKind: INodeKind<ImageNode> = {
  kind: 'image',
  find: (graph, id) => graph.images.find(i => i.id === id) ?? null,
  effectiveRect(ledger, img) {
    const e = ledger.effective(img);
    return { x: e.x, y: e.y, width: e.width, height: e.height };
  },
  move(ledger, img, dxPt, dyPt, pageW, pageH) {
    const e = ledger.effective(img);
    ledger.patchRect(img, {
